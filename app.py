...
def verify_gumroad_key(key):
    """Contacts the official Gumroad API endpoint to verify license keys."""
    try:
...
# DETAILED INFORMATIONAL EXPANDER MENU
...
# Interactive Real-World Benchmarks
...
if source == "Upload Custom CSV File":
    ...
    st.markdown("🔬 **Physical Baseline:** This dataset simulates the 2D Ising Model near its magnetic phase transition. *Expected Target Constants: Critical Temperature ($T_c$) $\\approx 2.2691$ | Exponent ($\\nu$) $= 1.0000$.*")
    ns_mock = [4, 8, 16, 32]
    rows = [{"n": n, "g": t, "K": 1 / (1 + np.exp((t - 2.2691) * n))} for n in ns_mock for t in np.linspace(2.0, 2.5, 20)]
    df_raw = pd.DataFrame(rows)
elif source == "Model Example: NASA Kepler Planet Radii Power-Law (2D)":
    st.markdown("🔭 **Astronomical Baseline:** This dataset maps planet sizes against system sizes from the NASA Kepler Archive. *Expected Target Constants: Scaling Exponent (Slope) $= -0.8500$.*")
    ns_mock = [10, 20, 40, 80, 160]
    df_raw = pd.DataFrame({"n": ns_mock, "g_peak": [5000 / (x ** 0.85) for x in ns_mock]})

# System Processing Cores
if analysis_type == "Premium: 3D FSS Curve Collapse" and not is_premium:
    st.error("Premium feature locked. Please provide your authorized license token in the sidebar panel to enable the 3D Engine.")
    ...
    if analysis_type == "Free: 2D Power-Law Fit":
        ...
        if st.button("🚀 Run Regression"):
            input_df = pd.DataFrame({"n": df_raw[n_col], "g_peak": df_raw[g_col]})
            res = run_2d_regression_engine(input_df)
            if "error" in res: st.error(res["error"])
                ...
                ax.scatter(input_df["n"], input_df["g_peak"], color="blue", alpha=0.7)
                ax.plot(input_df["n"], res["pred_g"], color="red", linestyle="--")
                ax.set_xscale("log")
                ax.set_yscale("log")
                ax.set_xlabel("System Size (n) - Log Scale")
                ax.set_ylabel("Peak Control Value (g_peak)")
        ...
        if st.button("🚀 Run 3D Collapse"):
            ...
            for n_v in sorted_bins:
                ...
                ax1.scatter(sub["g"], sub["K"], alpha=0.6, label=f"n={n_v}")
