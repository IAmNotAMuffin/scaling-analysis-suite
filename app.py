...
import requests
import guide # This calls your heavy text vault directly from your folder!
...
def run_2d_regression_engine(df):
    ...
    if "n" not in df.columns or "g_peak" not in df.columns: return {"error": "MISSING_COLUMNS"}
    n, g = df["n"].values, df["g_peak"].values
    if len(n) < 4 or np.any(n <= 0) or np.any(g <= 0): return {"error": "INVALID_DATA"}
    ...
    return {"slope": slope, "r2": r2, "confidence": confidence, "regime": "STRONG_SCALING" if r2 > 0.9 else "WEAK_SCALING", "n": n, "g": g, "pred_g": np.exp(pred)}
...
def run_3d_fss_engine(df):
    ...
    return {"gc": float(result.x[0]), "nu": float(result.x[1]), "score": float(score), "verdict": "STRONG SCALING" if score > 0.8 else "WEAK SCALING"}
...
# SHORT SHORT SHORT DISPLAY LOGIC PULLS FROM VAULT
with st.expander("📖 Quick User Guide & CSV Formatting Rules", expanded=False):
    st.markdown(guide.RULES_2D)
    st.code("n,g_peak\n5,1.5\n10,2.9\n20,5.8\n40,12.1", language="text")
    st.markdown(guide.RULES_3D)
...
if license_key:
    with st.sidebar.spinner("Verifying token status..."):
        if verify_gumroad_key(license_key):
            ...
            is_premium = True
        else: st.sidebar.error("❌ Invalid Key Token")
else:
...
# Real-World Benchmarks (Generated dynamically via math loops)
...
if source == "Upload Custom CSV File":
    ...
    st.markdown("🔬 **Physical Baseline:** Simulated 2D Ising Model. Expected: $T_c \\approx 2.2691$, $\\nu = 1.0000$.")
    ...
    st.markdown("🔭 **Astronomical Baseline:** Kepler Archive planetary radii distribution. Expected Exponent: $-0.8500$.")
    ...
    if analysis_type == "Free: 2D Power-Law Fit":
        ...
        if st.button("🚀 Run Regression"):
            ...
            if "error" in res: st.error(res["error"])
                ...
                st.download_button(label="📥 Download 2D Report", data=str(res), file_name="scaling_suite_2d_report.txt")
        ...
        if st.button("🚀 Run 3D Collapse"):
            ...
            c1, c2, c3 = st.columns(3)
            c1.metric("Critical Point (g_c)", round(res["gc"], 4), delta="2.2691 (Onsager Exact)" if "Ising" in source else None)
            c2.metric("Exponent ν", round(res["nu"], 4), delta="1.0000 (Onsager Exact)" if "Ising" in source else None)
            c3.metric("Collapse Score", round(res["score"], 4))
            st.success(res["verdict"])
            st.download_button(label="📥 Download 3D Report", data=str(res), file_name="scaling_suite_3d_report.txt")
            ...
            ax1.set_title("Unscaled Curves"); ax2.set_title("Optimized Collapse")
            st.write(fig)
            plt.close('all')
